...
dat = readtable("PV_heat_vm.csv");
shp = shaperead("tl_2019_50_tract.shp");
city =[];
for i = 1:length(shp)
    city = [city,str2num(shp(i).TRACTCE)];
end
% Initialize an array to store the polygon ID for each point
...
census = readtable("census_vm.csv");
shp_GEOID = [];
for i = 1:length(shp)
...
% idx = [];
% for i = 1:height(census)
%     int = sum(shp_GEOID == census.GEOID(i)*10);
%     if int == 1
%         idx = [idx;1];
%     else
%         idx = [idx;0];
...
for i = 1:length(shp_GEOID)
    idx(i,1) = find(census_GEOID == shp_GEOID(i,1));
end
...
output_shapefile = "tl_2019_53_tract_vm_census.shp";
